...
const AnalyzeResumeAtsOutputSchema = z.object({
  ...
  areasForImprovement: z.string().describe('Specific areas for improvement in the resume to better match the job description.'),
});
...
const analyzeResumeAtsPrompt = ai.definePrompt({
  ...
  prompt: `You are an expert ATS analyst. Analyze the resume against the job description and provide an ATS score (0-100) and identify areas for improvement.
...
ATS Score and Areas for Improvement:
`,config: {
    safetySettings: [
      {
